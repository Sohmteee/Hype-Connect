...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!paymentInit || !paymentInit.status) {
      ...
      await bookingRef.update({ status: 'failed', error: paymentInit?.message || 'Payment init failed' });
      return NextResponse.json({ success: false, error: 'Failed to initialize payment' }, { status: 500 });
    }

    // Update booking with payment reference
    await bookingRef.update({ paymentReference: paymentInit.data.reference, paymentUrl: paymentInit.data.authorization_url });

    return NextResponse.json({ success: true, data: { paymentUrl: paymentInit.data.authorization_url, reference: paymentInit.data.reference } });
  } catch (error) {
